...
def coef_heatmap(
    ...
    motifs: Iterable[str],
    *,
    phenotypes: Sequence[str] = ("Naive_High", "Naive_Low", "CM_High", "CM_Low", "EM_High", "EM_Low"),
    value: str = "z",
    cmap: str = "RdBu_r",
    center: float = 0.0,
    ...
    phenotypes = [normalize_phenotype(ph) for ph in phenotypes]
...
def pooled_coef_heatmap(
    ...
    *,
    ...
    - T-subsets (Naive, CM, EM): averaged over PD1 levels
    ...
    motifs = list(motifs)
    # Normalize tsubset and pd1 names
    tsubsets = [normalize_phenotype(ts) for ts in tsubsets]
    pd1_levels = [normalize_phenotype(pd1) for pd1 in pd1_levels]

    # Get covariance matrix for proper SE computation
    cov = np.asarray(fit.res.cov_params())
